    ...
    public static class UiManagerMethodsGenerator
    {
        ...
        public static bool Generate()
        {
            ...
            try
            {
                ...
                foreach (var fileInfo in fileInfos)
                {
                    if (fileInfo.Extension == ".tscn")
                    {
                        ...
                        methodClass += $"    /// <summary>\n" +
                                       ...
                                       $"    /// </summary>\n" +
                                       $"    public static {UiManager.UiNamespace}.{uiName}.{uiName}Panel Create_{uiName}()\n" +
                                       $"    {{\n" +
                                       $"        return CreateUi<{UiManager.UiNamespace}.{uiName}.{uiName}Panel>(UiName.{uiName});\n" +
                                       $"    }}\n" +
                                       $"\n" +
                                       $"    /// <summary>\n" +
                                       $"    /// 打开 {uiName}, 并返回UI实例\n" +
                                       $"    /// </summary>\n" +
                                       $"    public static {UiManager.UiNamespace}.{uiName}.{uiName}Panel Open_{uiName}()\n" +
                                       $"    {{\n" +
                                       $"        return OpenUi<{UiManager.UiNamespace}.{uiName}.{uiName}Panel>(UiName.{uiName});\n" +
                                       $"    }}\n" +
                                       $"\n" +
                                       $"    /// <summary>\n" +
                                       $"    /// 隐藏 {uiName} 的所有实例\n" +
                                       ...
                                       $"    /// </summary>\n" +
                                       $"    public static {UiManager.UiNamespace}.{uiName}.{uiName}Panel[] Get_{uiName}_Instance()\n" +
                                       $"    {{\n" +
                                       $"        return GetUiInstance<{UiManager.UiNamespace}.{uiName}.{uiName}Panel>(nameof({UiManager.UiNamespace}.{uiName}.{uiName}));\n" +
                                       $"    }}\n" +
                                       $"\n";
                    }
                }
                ...
                File.WriteAllText(UiManager.UiManagerMethodsCodePath, code);
            }
            catch (Exception e)
            {
